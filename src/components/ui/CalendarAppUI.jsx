...
/* ============================================================
   SHARED MAC OS UI COMPONENTS
   ============================================================ */

export function MacWindowFrame({ children, dark = false }) {
  return (
    <div className={`rounded-2xl overflow-hidden border border-[#E4E4E7] shadow-2xl shadow-black/10 ${dark ? "bg-[#0E0B20]" : "bg-white"}`}>
  ...
  return (
    <div className="rounded-xl border border-[#E4E4E7] bg-white p-2.5">
      ...
      <div className="grid grid-cols-7 gap-y-1 text-center">
        {days.map((d, i) => (
          <span key={`d-${i}`} className="text-[9px] text-[#A1A1AA] font-medium">{d}</span>
        ))}
        {weeks.flat().map((n, i) => {
...
export function LeftSidebar() {
  return (
    <div className="hidden md:flex md:flex-col w-[210px] shrink-0 bg-[#F7F6F8] border-r border-[#E4E4E7] p-3 gap-4 text-left">
      <div>
        <p className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wide mb-2 px-1">Calendars</p>
        <ul className="space-y-1.5">
          {[
            ...
            <li key={item.label} className={`flex items-center gap-2 px-2 py-1 rounded-md text-[11px] ${item.active ? "text-[#1A1033] font-semibold" : "text-[#71717A]"}`}>
              <span className="w-2 h-2 rounded-full border border-[#D4D4D8]" style={{ backgroundColor: item.dot }} />
              {item.label}
            </li>
          ))}
        </ul>
      ...
      <div>
        <p className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wide mb-2 px-1">People</p>
        <div className="rounded-md bg-white border border-[#E4E4E7] px-2 py-1 text-[10px] text-[#A1A1AA] mb-2">Search for someone</div>
        <ul className="space-y-1.5">
          {[
...
export function RightSidebar() {
  ...
  return (
    <div className="hidden lg:flex lg:flex-col w-[220px] shrink-0 bg-white border-l border-[#E4E4E7] p-4 text-left">
